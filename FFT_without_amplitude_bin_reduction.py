...
This python script is my Delia Fano Yela's implementation of FFT algorithm.
I used this to show the robustness of the algorithm with my own data
        ...
        for subdir, dirs, files in os.walk(dir):
            for i, filename in enumerate(files):
                ...
                for index in div:
                    ...
                    # NVG DOMAIN ---------------------------------------------------
                    sn = nvg_dc(series = list(sa) , timeLine = range(N), left = 0, right = N)
                    mn = nvg_dc(series = list(ma) , timeLine = range(N), left = 0, right = N)
...
#df_mrr.to_csv ('df_mrr_fft.csv', index = None, header=True)
#df_mrr.to_csv ('df_mrr_fft_my_data.csv', index = None, header=True)
